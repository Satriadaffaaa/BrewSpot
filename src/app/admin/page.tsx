...
export default function AdminPage() {
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-12">
            ...
            <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#1a1a2e] to-[#2c1e16] p-8 md:p-10 shadow-2xl border border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {isLoading ? (
                    [1, 2, 3].map(i => <div key={i} className="h-40 bg-white/50 rounded-2xl animate-pulse border border-white/20" />)
                ) : (
                    statCards.map((card, idx) => (
                    <Link key={idx} href={card.href} className="group block focus:outline-none">
                        <div className="relative overflow-hidden rounded-2xl bg-white p-5 shadow-sm border border-[#2c1e16]/5 transition-all duration-500 hover:shadow-2xl hover:border-[#E8A87C]/20 hover:-translate-y-1">
                            <div className={cn("absolute right-6 top-6 rounded-2xl p-3", card.bg)}>
                                <card.icon className={cn("h-6 w-6", card.color)} />
                            </div>

                            <div className="space-y-1">
                                <p className="text-xs font-black uppercase tracking-widest text-[#8B5E3C] group-hover:text-[#E8A87C] transition-colors">{card.title}</p>
                                <p className="text-5xl font-black font-heading text-[#2c1e16] tracking-tighter">{card.value}</p>
                            </div>
                            <div className="mt-4 flex items-center text-[10px] font-black uppercase tracking-widest text-[#8B5E3C]/70">
                    ...
                    <Link href="/admin/analytics" className="group p-4 rounded-2xl bg-white border border-[#2c1e16]/5 hover:border-[#E8A87C]/20 transition-all duration-300">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-[#8B5E3C]/80">Traffic & Growth Metrics</p>
                            </div>
                        </div>
                    </Link>

                    <Link href="/admin/ai-tools" className="group p-4 rounded-2xl bg-white border border-[#2c1e16]/5 hover:border-[#E8A87C]/20 transition-all duration-300">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-[#8B5E3C]/80">AI Quotas & Audit Logs</p>
                            </div>
                        </div>
                    </Link>
                    </div>
                    ...
                    {/* System Health */}
                    <div className="rounded-[2rem] bg-white p-6 border border-[#2c1e16]/5 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-lg font-black text-[#2c1e16] uppercase tracking-tight">System Health</h3>
                            <p className="text-xs text-[#8B5E3C]/80">Live Operational Status</p>
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {[
                            ...
                            <div key={i} className="p-4 rounded-2xl bg-[#F5E6D3]/30 border border-[#2c1e16]/5 flex flex-col items-center text-center">
                                ...
                                <span className="text-[8px] font-black text-[#8B5E3C]/70 uppercase tracking-[0.2em] mb-1">{item.label}</span>
                ...
                <div className="space-y-6">
                    <div className="rounded-[2rem] bg-[#1a1a2e] p-6 border border-white/5 shadow-2xl flex flex-col h-full">
                        <div className="mb-8">
