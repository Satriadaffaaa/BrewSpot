    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold font-heading text-primary">Pending Reviews</h1>
                    <p className="text-neutral-500 mt-1">{spots.length} applications waiting for your decision</p>
                </div>

                {/* Global Safety Switch */}
                {settings && (
                    <div className="flex items-center gap-4 bg-surface px-5 py-3 rounded-2xl shadow-sm border border-border/60 transition-all hover:shadow-md">
                        ...
                        >
                            ...
                            <span
                                className={`inline-block h-5 w-5 transform rounded-full bg-surface shadow-sm transition-transform duration-200 ease-spring ${settings.enableAutoApproval ? 'translate-x-[22px]' : 'translate-x-0.5'
                                    }`}
                            />
                        </button>
                    </div>
            ...
            {spots.length === 0 ? (
                <div className="bg-surface rounded-3xl p-16 text-center shadow-sm border border-neutral-100">
                    ...
                    <p className="text-neutral-500 max-w-sm mx-auto">There are no pending spot applications requiring moderation at this time.</p>
                </div>
            ) : (
