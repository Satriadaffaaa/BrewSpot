                ...
                {spots.map((spot) => (
                    <div key={spot.id} className="bg-surface p-4 rounded-xl shadow-sm border border-neutral/10 flex flex-col gap-3">
                        ...
                        <Link href={`/admin/spots/${spot.id}`} className="w-full">
                            <Button size="sm" className="w-full justify-center bg-surface text-neutral-600 border border-border hover:bg-primary hover:text-white hover:border-primary">
                                Review Application
                            </Button>
                        </Link>
                    </div>
                    ...
                    <tbody className="divide-y divide-neutral-100">
                        {spots.map((spot) => (
                            <tr key={spot.id} className="group hover:bg-neutral-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm font-medium text-neutral/70 bg-neutral-100 px-3 py-1 rounded-full w-fit">
                                        {spot.city}
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <Link href={`/admin/spots/${spot.id}`}>
                                        <Button size="sm" className="bg-surface text-neutral-600 border border-border hover:bg-primary hover:text-white hover:border-primary shadow-sm active:scale-95">
                                            Review
                                        </Button>
                                    </Link>
                                </td>
